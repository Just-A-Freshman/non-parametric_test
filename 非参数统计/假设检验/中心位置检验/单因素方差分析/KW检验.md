...
他将所有不同组的数据数据全部混合后映射到秩，随后仿照参数统计的方差分析思想构造统计量。

需要注意，混合秩次对于分布形状是敏感的。例如两组中心位置完全相同的分布，一组左偏，一组右偏。混合编秩后，左偏分布的秩和就会明显小于右偏分布的秩次。因此，只要是以检验**位置参数差异**为目的的**混合编秩**检验，都要求每组数据的分布具有相似的形状(偏度、峰度)。

## 2 统计量
### 2.1 形式
记$N$为总样本量，$k$为总组数，$\tau_j$为第$j$个结的结长，$\bar{R_i}$为第$i$组样本的平均秩，$R_{i.}$为第$i$组的秩和。那么KW的检验统计量如下：
$$
H = \frac{SSA}{MST}
$$只需再结合以下计算结果：
$$\begin{aligned}
&SSA = \sum_{i=1}^k\sum_{j=1}^{n_i} (\bar{R_i} - \bar{R})^2 = \sum_{i=1}^k n_i \bar{R_i}^2 - \frac{N(N + 1)^2}{4} \\
&MST = \frac{\sum_{i=1}^k\sum_{j=1}^{n_i} (R_{ij} - \bar{R})^2}{N - 1} = \frac{N(N + 1)}{12} \\
...
$$
\frac{SSA}{MST} = \frac{\sum_{i=1}^k (\bar{R_i} - \bar{R})^2}{\sigma^2} = \sum_{i=1}^k \left(\frac{\bar{R_i} - \bar{R}}{\sigma}\right)^2
...
回顾参数统计中方差分析的多重比较。因为：
$$
\bar{x_{i.}} - \bar{x_{j.}} \sim N(\mu_i - \mu_j, \left(\frac{1}{n_i} + \frac{1}{n_j}\right)\sigma^2)
$$所以：
$$
\frac{(\bar{x_{i.}} - \bar{x_{j.}}) - (\mu_i - \mu_j)}{\sqrt{\left(\frac{1}{n_i} + \frac{1}{n_j}\right)\sigma^2}} \sim N(0, 1)
...
$$
\frac{(\bar{x_{i.}} - \bar{x_{j.}}) - (\mu_i - \mu_j)}{\sqrt{\left(\frac{1}{n_i} + \frac{1}{n_j}\right)MSE}} \sim t(f_e)
$$
...
因此：
$$
\frac{(\bar{R_i} - \bar{R_{j}}) - (M_i - M_j)}{\sqrt{\left(\frac{1}{n_i} + \frac{1}{n_j}\right) MST}} \sim N(0, 1)
$$
